...
\label{sec:hidden_valley_model}
	...
	\label{sub:training_datasets}
		The total cross-section of the background events is $6837 \text{ fb}^{-1}$. From Table~\ref{tab:HVmodel_cutflow_number}, we can compute the corresponding cross-sections of signal and sideband regions are $136.1 \text{ fb}^{-1}$ and $211.2 \text{ fb}^{-1}$, respectively. Thus, the numbers of events in signal and sideband region are $19k$  and $29k$ at luminosity $\mathcal{L} = 139 \text{ fb}^{-1}$.
