...
\label{sec:hidden_valley_model}
	...
	\label{sub:remove_rectification}
		...
		Figure~\ref{fig:acc_curve_w_wo_swap_label} is accuracy curves with different training datasets. Figure~\ref{fig:acc_scatter_w_wo_swap_label} is accuracy scatter plots with different training datasets. The training results with and without swapping procedures are similar for all cases. It seems that the rectification process does not have a large impact on the performance. The rectification is not the main reason for the improvement.
		\begin{figure}[htpb]
			\centering
			\subfloat[Original]{
