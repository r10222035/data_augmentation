...
\label{sec:hidden_valley_model}
	...
	\label{sub:jet_image}
		We can construct the jet image from the event passing the selection cuts described in Section~\ref{sub:sample_selection}. The jet image is constructed for each jet separately, so we would obtain two for each event. They are treated as two channels of a picture. The following steps construct the jet image:
		\begin{enumerate}
	...
	\label{sub:tensorflow_model_compile}
		...
		For the previous model structure, Tensorflow would use CategoricalAccuracy, which checks the index of output whether it is the same as the index of the true label.
	...
	\label{sub:smearing_scale_in_eta_phi_smearing}
		...
		Figure~\ref{fig:sensitivity_improvement_aug_1_smearing_scale} shows the sensitivity improvement with various smearing scales. The training results are similar for all cases. It seems that the $\eta-\phi$ augmentation can not improve the training performance.
		\begin{figure}[htpb]
			\centering
			\subfloat[Resolution: $75\times 75$]{
	...
	\label{sub:more_augmented_sample}
		...
		Figure~\ref{fig:sensitivity_improvement_origin_eta_phi_aug_1_3} shows the sensitivity improvement with larger $\eta-\phi$ smearing datasets. Even the we enlarge the training sample size, the training results are still similar. It seems that the $\eta-\phi$ augmentation can not improve the training performance.
		\begin{figure}[htpb]
			\centering
			\subfloat[Resolution: $75\times 75$]{
	...
	\label{sub:jet_rotation}
		...
		\begin{enumerate}
			...
			\item Rotate each jet in each event with a random angle. The rotation angle is sampled from $[-\pi, \pi]$ uniformly.
