...
\label{sec:hidden_valley_model}
	...
	\label{sub:hidden_valley_model_training_results}
		The CNN is trained on samples with sensitivity $S / \sqrt{B}$ ranging from 1 to 10. Figure~\ref{fig:cwola_hunting_cnn_acc} presents the training results. These numbers are evaluated from the pure samples, which consist of 10k signal events and 10k background events. The CNN cannot learn useful information for the case with a sensitivity $S / \sqrt{B} \le  5$, so the ACC is 0.5. After this region, the accuracy demonstrates a significant improvement. It seems that the CNN model surpasses a certain threshold.
	...
	\label{sub:data_process_procedure}
		\begin{enumerate}
			...
			\item Apply the selection cuts described in Section~\ref{sub:sample_selection} and save the event passing the cuts in \verb|HDF5| format. The file contains the information listed below 
				...
				\end{itemize}
			\item Make mixed sample in \verb|HDF5| format. Following Section~\ref{sub:training_datasets}, we can compute the size of datasets.	
	...
	\label{sub:data_augmentation}
		...
		We generate samples with sensitivity $S / \sqrt{B}$ ranging from 1 to 10. Then, apply the data augmentation to make larger samples. These samples are used for CNN training. Figure~\ref{fig:cwola_hunting_cnn_acc_data_augmetation} presents the training results. These numbers are evaluated from the pure samples, which consist of 10k signal events and 10k background events. The ACC is better than previous results for the ``+1 times'' curve (Figure~\ref{fig:cwola_hunting_cnn_acc}) for all sensitivities. It seems that the data augmentation technique can suppress the threshold and improve the training performance.
