...
\label{sec:hidden_valley_model}
	...
	\label{sub:jet_image_normalization}
		...
		First, compute the total invariant mass $m_{jj}$ for the event passing the pre-selection cuts. Then, use the $m_{jj}$ to normalize the transverse momentum $p_{\text{T}}$ of each jet constituent:
		\begin{equation}
		...
		Figure~\ref{fig:average_jet_image_origin_pt_normalized_difference} is the original and $p_{\text{T}}$ normalized jet images. Here, we show the jet images of background events in the SB region. The $p_{\text{T}}$ normalized and original cases have a similar distribution, but the values are not the same.
        \begin{figure}[htpb]
            \centering
			\subfloat[$p_{\text{T}}$ normalized]{
                \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_pt_normalized.pdf}
            }
            \subfloat[Original]{
                \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_origin.pdf}
            } \\ 
            \subfloat[Difference]{
                \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_pt_normalized_origin_difference.pdf}
            }
            \caption{The $p_{\text{T}}$ normalized, original, and their difference average jet images.}
		...
		There are obvious sculpting issues for all the background efficiencies. The $p_{\text{T}}$ normalization makes the situation worse than previous cases (Figure~\ref{fig:mjj_distribution_origin_jet_aug_3_pt_jet_aug_3_train}).
        \begin{figure}[htpb]
            \centering
			\subfloat[Original, $p_{\text{T}}$ normalized, $S / \sqrt{B} = 0$]{
                \includegraphics[width=0.97\textwidth]{mjj_distribution_CNN_cut_pt_normalized_origin_SB_0_train.pdf}
            }
            \caption{The $m_{jj}$ distribution before and after the CWoLa CNN selection. The signal region is between the red dotted lines. The sideband region is between the green dotted lines and excludes the signal region. ``B'' stands for the background samples.}
            \label{fig:mjj_distribution_pt_normalized_origin_train}
        \end{figure}

		 Figure~\ref{fig:sensitivity_improvement_background_subtraction_larger_testing_data} shows the sensitivity improvement with $p_{\text{T}}$ normalization technique. The training thresholds are much higher than the original cases. However, when we focus on the results at sensitivity 0 with $\varepsilon_{\text{b}} = 10\%$. The sensitivity improvement is $2.63 \pm 0.92$ for original method, $0.71 \pm 1.00$ for $p_{\text{T}}$ normalization. 
        \begin{figure}[htpb]
            \centering
