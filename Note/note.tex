...
\label{sec:physical_data_augmentation}
    ...
    \label{sub:physical_augmentation}
        ...
        For each type of augmentation, we test ``$n$ times augmentation'' with different $n$. The $n$ times augmentation means that for one original sample, we generate $n$ augmented samples. Additionally, we test another case that applies all augmentations at the same time.
...
\label{sec:hidden_valley_model}
    ...
    \label{sub:background_subtraction}
        ...
        Figure~\ref{fig:train_background_passing_rate} shows the background efficiencies of training datasets. The passing rate of training sets is consistently higher than that of testing sets. This satisfied our expectations, the neural network would prefer to choose the training samples.
        \begin{figure}[htpb]
            \centering
    ...
    \label{sub:k_fold_cross_validation}
        ...
        Figure~\ref{fig:sensitivity_improvement_background_pass_rate_k_fold} shows the training results with the k-fold technique. The training performance with and without the k-fold technique is similar. The k-fold technique only reduces the training fluctuation.
        \begin{figure}[htpb]
            \centering
            \subfloat[Sensitivity improvement]{
    ...
    \label{sub:another_testing_dataset}
        ...
        \end{figure}
    % subsection another_testing_dataset (end)
    \subsection{Enlarge the testing dataset}% (fold)
    \label{sub:enlarge_the_testing_dataset}
        ...
        \begin{figure}[htpb]
            \centering
            \subfloat[Sensitivity improvement]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_origin_bkg_subtraction_ZN_pool_larger_testing_set.pdf}
            }
            \caption{The sensitivities before and after the CWoLa CNN selection. The slope of the dashed grey line is $1$, representing the same performance before and after the selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
        ...
        \end{figure}
    % subsection enlarge_the_testing_dataset (end)
    \subsection{Jet image normalization}% (fold)
    \label{sub:jet_image_normalization}
        ...
        \subsubsection{Mass normalization}% (fold)
        \label{subs:mass_normalization}
            ...
            \begin{figure}[htpb]
                \centering
                \subfloat[$m_{jj}$ normalized]{
                    \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_mjj_normalized.pdf}
                }
                \subfloat[Original]{
                    \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_origin.pdf}
                } \\ 
                \subfloat[Difference]{
                    \includegraphics[width=0.45\textwidth]{HVmodel_jet_image_average_bkg_in_SB_mjj_normalized_origin_difference.pdf}
                }
            ...
            \begin{figure}[htpb]
                \centering
                \subfloat[Original, $m_{jj}$ normalized, $S / \sqrt{B} = 0$]{
                    \includegraphics[width=0.97\textwidth]{mjj_distribution_CNN_cut_mjj_normalized_origin_SB_0_train.pdf}
                }
                \caption{The $m_{jj}$ distribution before and after the CWoLa CNN selection. The signal region is between the red dotted lines. The sideband region is between the green dotted lines and excludes the signal region. ``B'' stands for the background samples.}
            ...
            \end{figure}
        % subsubsection mass_normalization (end)    
        \subsubsection{SRSB normalization}% (fold)
        \label{subs:srsb_normalization}
            ...
            \begin{figure}[htpb]
                \centering
                \subfloat[Original, SRSB normalized, $S / \sqrt{B} = 0$]{
                    \includegraphics[width=0.97\textwidth]{mjj_distribution_CNN_cut_SRSB_normalized_origin_SB_0_train.pdf}
                }
                \caption{The $m_{jj}$ distribution before and after the CWoLa CNN selection. The signal region is between the red dotted lines. The sideband region is between the green dotted lines and excludes the signal region. ``B'' stands for the background samples.}
            ...
            \end{figure}
        % subsubsection srsb_normalization (end)
        \subsubsection{\texorpdfstring{$p_{\mathrm{T}}$}{pT} normalization}% (fold)
        \label{subs:pt_normalization}
            ...
            \begin{figure}[htpb]
                \centering
                \subfloat[Original, image $p_{\text{T}}$ normalized, $S / \sqrt{B} = 0$]{
                    \includegraphics[width=0.97\textwidth]{mjj_distribution_CNN_cut_image_pT_normalized_origin_SB_0_train.pdf}
                } \\
                \subfloat[Original, event $p_{\text{T}}$ normalized, $S / \sqrt{B} = 0$]{
                    \includegraphics[width=0.97\textwidth]{mjj_distribution_CNN_cut_event_pT_normalized_origin_SB_0_train.pdf}
                }
                \caption{The $m_{jj}$ distribution before and after the CWoLa CNN selection. The signal region is between the red dotted lines. The sideband region is between the green dotted lines and excludes the signal region. ``B'' stands for the background samples.}
            ...
            \end{figure}
        % subsubsection pt_normalization (end)
        \subsubsection{\texorpdfstring{$p_{\mathrm{T}}$}{pT} normalization 2}% (fold)
        \label{subs:pt_normalization_2}
            ...
            \end{figure}
        % subsubsection pt_normalization_2 (end)	
    % subsection jet_image_normalization (end)
    \subsection{Data augmentation with image normalization}% (fold)
    \label{sub:data_augmentation_with_image_normalization}
        We enlarge the training dataset using data augmentation techniques to determine whether the training performance can be improved when we combine the image normalization. We test ``Jet rotation'' and ``$p_{\text{T}}$ smearing + Jet rotation'' augmentation methods.
        ...
        \begin{figure}[htpb]
            \centering
            \subfloat[Image normalization]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_image_pT_normalied_origin_jet_aug_pt_jet_aug_5_10.pdf}
            } \\
            \caption{The sensitivities before and after the CWoLa CNN selection. The slope of the dashed grey line is $1$, representing the same performance before and after the selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
        ...
        \begin{figure}[htpb]
            \centering
            \subfloat[Image normalization]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_image_pT_normalied_aug_3_to_33_SB_3.pdf}
            } \\
        ...
        \end{figure}
    % subsection data_augmentation_with_image_normalization (end)
    \subsection{Data augmentation with event normalization}% (fold)
    \label{sub:data_augmentation_with_event_normalization}
        ...
        \begin{figure}[htpb]
            \centering
            \subfloat[Event normalization]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_origin_jet_aug_pt_jet_aug_5_10.pdf}
            } \\
            \caption{The sensitivities before and after the CWoLa CNN selection. The slope of the dashed grey line is $1$, representing the same performance before and after the selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
        ...
        \begin{figure}[htpb]
            \centering
            \subfloat[Event normalization, Sensitivity: 3.0]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_aug_3_to_33_SB_3.pdf}
            } \\
            \subfloat[Event normalization, Sensitivity: 4.0]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_aug_3_to_33_SB_4.pdf}
            } \\
            \subfloat[Event normalization, Sensitivity: 5.0]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_aug_3_to_33_SB_5.pdf}
            } \\
            \caption{The sensitivities after the CWoLa CNN selection. Here, $3.0$ is the sensitivity before selection. The dashed line is the sensitivity before CNN selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
            \label{fig:sensitivity_improvement_aug_SB_3_4_5_event_norm}
        \end{figure}
    % subsection data_augmentation_with_event_normalization (end)
    \subsection{Event normalization with Dropout}% (fold)
    \label{sub:event_normalization_with_dropout}
        ...
        The dropout layer is added after each original layer with a dropout rate 0.35. Figure~\ref{fig:sensitivity_improvement_origin_pt_jet_aug_10_event_norm_dp_035} presents the sensitivity improvement with and without dropout layers. For the original dataset, the model with dropout performs worse than the model without dropout, and the fluctuations are greater than in the previous cases. For +10 augmentation, the training performance is better than the previous one in the high-sensitivity region, while the training thresholds and the standard deviations are similar to the earlier results.
        \begin{figure}[htpb]
            \centering
            \subfloat[Event normalization]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_origin_jet_aug_pt_jet_aug_10.pdf}
            } \\
            \caption{The sensitivities before and after the CWoLa CNN selection. The slope of the dashed grey line is $1$, representing the same performance before and after the selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
            \label{fig:sensitivity_improvement_origin_pt_jet_aug_10_event_norm_dp_035}
        \end{figure}

        Figure~\ref{fig:sensitivity_improvement_aug_SB_3_event_norm_dp_035} shows the sensitivity improvement with different augmented sample sizes. If the augmented sample size is large enough, then the model with dropout could perform better than the previous one.
        \begin{figure}[htpb]
            \centering
            \subfloat[Event normalization, Sensitivity: 3.0]{
                 \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_aug_3_to_33_SB_3.pdf}
            } \\
            \subfloat[Event normalization, Sensitivity: 5.0]{
                 \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_aug_3_to_33_SB_5.pdf}
            } \\
            \caption{The sensitivities after the CWoLa CNN selection. Here, $3.0$ is the sensitivity before selection. The dashed line is the sensitivity before CNN selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
            \label{fig:sensitivity_improvement_aug_SB_3_event_norm_dp_035}
        \end{figure}

        Figure~\ref{fig:sensitivity_improvement_aug_SB_3_5_event_norm_dp_035_aug_50} shows the sensitivity improvement with different augmented sample sizes. The ``$p_{\text{T}}$ smearing + jet rotation'' performs better than jet rotation. The sensitivity improvement of both augmentation methods is saturated after +33 augmentation.
        \begin{figure}[htpb]
            \centering
            \subfloat[Event normalization, Sensitivity: 3.0]{
                 \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_aug_3_to_50_SB_3.pdf}
            } \\
            \subfloat[Event normalization, Sensitivity: 5.0]{
                 \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_aug_3_to_50_SB_5.pdf}
            } \\
            \caption{The sensitivities after the CWoLa CNN selection. Here, $3.0$ is the sensitivity before selection. The dashed line is the sensitivity before CNN selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training.}
            \label{fig:sensitivity_improvement_aug_SB_3_5_event_norm_dp_035_aug_50}
        \end{figure}
    % subsection event_normalization_with_dropout (end)
    \subsection{Batch size}% (fold)
    \label{sub:batch_size}
        In the previous exercises, we always set the batch size to 512. This batch size is a reasonable choice for the original dataset, which contains around 40k samples. However, increasing the sample size through the data augmentation technique makes it possible to increase the batch size as well. 

        We utilize +10 augmentation as an example and modify the batch size to investigate how different settings impact the training performance. Figure~\ref{fig:sensitivity_improvement_aug_10_event_norm_dp_035_batch_size} is the sensitivity improvement of various batch sizes. They have similar sensitivity improvement and fluctuations.
        \begin{figure}[htpb]
            \centering
            \subfloat[+10 jet rotation]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_jet_aug_10.pdf}
            } \\
            \subfloat[+10 $p_{\text{T}}$ smearing + jet rotation]{
                \includegraphics[width=0.97\textwidth]{HVmodel_sensitivity_improvement_event_pT_normalied_dp_035_pt_jet_aug_10.pdf}
            } 
            \caption{The sensitivities before and after the CWoLa CNN selection. The ``BS'' means batch size. The slope of the dashed grey line is $1$, representing the same performance before and after the selection. The error bar is the standard deviation of 10 times training. We use different datasets for each training. To improve the clarity of the error bars, we applied an offset to the error bars while keeping the main curve in its original position.}
            \label{fig:sensitivity_improvement_aug_10_event_norm_dp_035_batch_size}
        \end{figure}
