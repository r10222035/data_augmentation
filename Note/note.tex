...
\label{sec:hidden_valley_model}
	...
	\label{sub:data_augmentation}
		...
		We generate samples with sensitivity $S / \sqrt{B}$ ranging from 1 to 10. Then, apply the data augmentation to make larger samples. These samples are used for CNN training. Figure~\ref{fig:cwola_hunting_cnn_acc_data_augmetation} presents the training results. These numbers are evaluated from the pure samples, which consist of 10k signal events and 10k background events. For the ``+1 times'' curve, the ACC is better than previous results (Figure~\ref{fig:cwola_hunting_cnn_acc}) for all sensitivities. It seems that the data augmentation technique can suppress the threshold and improve the training performance.
