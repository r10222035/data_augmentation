...
\label{sec:hidden_valley_model}
	...
	\label{sub:original_x_copy}
		...
		Figure~\ref{fig:sensitivity_improvement_bkg_eff_01_copy_ratio} is the sensitivity improvement. For $\varepsilon_{\text{b}} = 10\%$, the $+ 0.50 \sim +1$ copy samples have similar thresholds which are lower than the original training sample, but the sensitivity improvement is smaller than the original dataset in the high sensitivity region. When the background efficiency is lower, the $+ 0.50$ copy and $+ 0.75$ copy samples perform better than other cases, even though they are over-training. 
		\begin{figure}[htpb]
			\centering
			...
			\label{fig:sensitivity_improvement_bkg_eff_01_copy_ratio}
		\end{figure}
